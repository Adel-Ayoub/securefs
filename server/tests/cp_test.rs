...
async fn test_recursive_copy() {
    ...
    dao::add_file(&pool, dir_node).await.expect("create src dir");
    dao::add_file_to_parent(&pool, "/home".into(), src_dir_name.clone()).await.expect("link src dir");
    ...
    dao::add_file(&pool, file1_node).await.expect("create file1");
    dao::add_file_to_parent(&pool, src_dir_path.clone(), "file1".into()).await.expect("link file1");
    ...
    dao::copy_recursive(&pool, src_dir_path.clone(), dst_dir_path.clone(), "admin".to_string())
        .await
    ...
    let dst_file1_node = dao::get_f_node(&pool, dst_file1_path.clone()).await.unwrap();
