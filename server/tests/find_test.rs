...
async fn test_find_command() {
    ...
    let match_target = FNode {
        id: -1,
        name: "match_target.txt".into(),
        path: "/home/dir1/match_target.txt".into(),
        owner: "admin".into(),
        hash: "".into(),
        parent: "/home/dir1".into(),
        dir: false,
        u: 7,
        g: 7,
        o: 7,
        children: vec![],
        encrypted_name: "match_target.txt".into(),
        size: 0,
        created_at: 0,
        modified_at: 0,
        file_group: None,
    ...
    dao::add_file(&pool, match_target)
        .await
        .expect("create match_target");
    dao::add_file_to_parent(&pool, "/home/dir1".into(), "match_target.txt".into())
        .await
        .expect("link match_target");

    let other = FNode {
        id: -1,
        name: "other.txt".into(),
        path: "/home/dir1/other.txt".into(),
        owner: "admin".into(),
        hash: "".into(),
        parent: "/home/dir1".into(),
        dir: false,
        ...
        if let Ok(Some(node)) = dao::get_f_node(&pool, search_path.clone()).await {
            ...
            if node.dir {
                for child in node.children {
                    let child_path = if search_path == "/" {
                        format!("/{}", child)
                    } else {
                        format!("{}/{}", search_path, child)
                    };
